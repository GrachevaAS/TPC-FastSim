...
def get_generator(activation, kernel_init, latent_dim):
    generator = tf.keras.Sequential([
        tf.keras.layers.Reshape((4, 4, latent_dim // 16), input_shape=(latent_dim,)),

        tf.keras.layers.Conv2D(filters=32, kernel_size=3, padding='same', activation=activation, kernel_initializer=kernel_init),
        tf.keras.layers.Conv2D(filters=32, kernel_size=3, padding='same', activation=activation, kernel_initializer=kernel_init),
        tf.keras.layers.Conv2DTranspose(32, kernel_size=3, strides=(2, 2), padding='same', use_bias=False),  # 8x8
        ...
        tf.keras.layers.Conv2DTranspose(16, kernel_size=3, strides=(2, 2), padding='same', use_bias=False),  # 12x12
...
class BaselineModel10x10:
    def __init__(self, activation=tf.keras.activations.relu, kernel_init='glorot_uniform',
        ...
        self.disc_opt = tf.keras.optimizers.Adam(lr, beta_1=0.5, beta_2=0.9)
        self.gen_opt = tf.keras.optimizers.Adam(lr, beta_1=0.5, beta_2=0.9)
